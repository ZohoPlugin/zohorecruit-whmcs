 ...
 */
function hook_zoho_recruit_clientedit(array $params)
{
    try {
 ...
 */
add_hook('ClientEdit', 1, 'hook_zoho_recruit_clientedit');
/**
    ...
    if (!is_null($menu->getChild('Services'))) {
        ...
        $menu->getChild('Services')
            ->addChild(
                'Zoho Recruit',
                array(
                    'uri' => 'clientarea.php?action=services&module=zoho_recruit',
                    'order' => 15,
                )
            );
    }
...
add_hook('ClientAreaSecondarySidebar', 1, function ($secondarySidebar)
{
    ...
    if (!is_null($secondarySidebar->getChild('My Services Actions'))) {
        ...
        $customPanel = $secondarySidebar->addChild('Zoho Recruit');
